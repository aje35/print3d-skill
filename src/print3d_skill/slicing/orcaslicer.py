...
class OrcaSlicerBackend(SlicerBackend):
    """OrcaSlicer CLI integration.

    OrcaSlicer is a PrusaSlicer fork with a compatible CLI interface.
    Detects ``orca-slicer`` or ``OrcaSlicer`` on PATH.
    """
    ...
    def slice(
        ...
        exe = self._find_executable()
        if exe is None:
            raise SlicerError("orcaslicer", message="OrcaSlicer not found on PATH")
        ...
        try:
            ...
            raise SlicerError(
                "orcaslicer",
                message=f"OrcaSlicer timed out after {_TIMEOUT_S}s",
            ) from exc
        finally:
            # Clean up temp INI
            if tmp_ini_path:
        ...
        if result.returncode != 0:
            raise SlicerError("orcaslicer", stderr=result.stderr)
        ...
        return SliceResult(
            ...
            slicer_used=SlicerType.ORCASLICER,
